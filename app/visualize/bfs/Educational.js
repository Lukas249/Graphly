...
const Educational = () => {
  ...
  useEffect(() => {
    ...
    links.forEach((link) => {
      adjacency[link.source.id ?? link.source].push(
        link.target.id ?? link.target,
      );
      adjacency[link.target.id ?? link.target].push(
    ...
    async function bfs(startId, visited = new Set()) {
      ...
      while (queue.length) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          label.filter((d) => d.id === id).attr("fill", "white");
  ...
  return (
    <div className="flex grow-1 flex-col items-center p-8">
