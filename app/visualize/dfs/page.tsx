...
export default function Visualize() {
    ...
    function formatEdgesForGraph(userEdges: Edges): Edge[] {
        ...
        userEdges.forEach((targets, source) => {
            targets.forEach((weight, target) => {
                ...
                result.push({source: {id: source}, target: {id: target}, directed: isDirected, weight})
            })
        })
    ...
    function parseNodes(userNodes: string): Nodes {
        return new Set(userNodes.split(/\s+/).filter(line => line.trim().length > 0))
    }
    ...
    function parseEdges(nodes: Nodes, userEdges: string): Edges {
        ...
        userEdges.split(/\s+/).filter(line => {
            ...
            return line.trim().length > 0 && source && target && nodes.has(source) && nodes.has(target)
        }).forEach(line => {
            const isUndirected = line.includes(edgeSeparator.get("undirected") ?? "")
    ...
    function stringifyEdges(userEdges: Edges): string {
        ...
        userEdges.forEach((targets, source) => {
            targets.forEach((weight, target) => {
                const isDirected = userEdges.get(source)?.has(target) != userEdges.get(target)?.has(source)
                ...
                result.push(`${source}${edgeSeparator.get(directed)}${target}:${weight}`)
            })
        })
    ...
    const setNodesDebounced = _.debounce((val: string | undefined) => setNodes(parseNodes(val || "")), 2000)
    const setEdgesDebounced = _.debounce((val: string | undefined) => setEdges(parseEdges(nodes, val || "")), 2000)

    return (
        <div className="flex flex-row">
            <GraphDFS nodes={formatNodesForGraph(nodes)} edges={formatEdgesForGraph(edges)}/>

            <div className="h-screen w-1/2 max-w-2xs flex flex-col p-8 gap-2">
